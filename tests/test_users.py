...
class TestAsyncDRACOONUsers(unittest.IsolatedAsyncioTestCase):
    ...
    def assert_user(self, user) -> None:
        assert user.id == 1
    ...
    async def test_get_users(self):
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users').respond(200, json=self.users_json)
    ...
    async def test_get_users_with_filter(self):
        filter = 'type:eq:file'
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users?offset=0&filter={filter}').respond(200, json=self.users_json)
    ...
    async def test_get_users_with_sort(self):
        sort = 'name:asc'
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users?offset=0&sort={sort}').respond(200, json=self.users_json)
    ...
    async def test_get_users_with_limit(self):
        limit = 1
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users?offset=0&limit={limit}').respond(200, json=self.users_json)
    ...
    async def test_get_users_with_offset(self):
        offset = 100
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users?offset={offset}').respond(200, json=self.users_json)
    ...
    async def test_get_users_with_roles(self):
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users?offset=0&include_roles=true').respond(200, json=self.users_json)
    ...
    async def test_get_users_with_attributes(self):
        get_users_mock = respx.get(
            f'{BASE_URL}/api/v4/users?offset=0&include_attributes=true').respond(200, json=self.users_json)
    ...
    async def test_create_user(self):
        create_user_mock = respx.post(
            f'{BASE_URL}/api/v4/users').respond(201, json=self.user_json)
        user_payload = self.users.make_local_user(
            first_name='test', last_name='test', email='test@dracoon.com', login='local.user')
    ...
    async def test_get_user(self):
        user_id = 1
        create_user_mock = respx.get(
            f'{BASE_URL}/api/v4/users/{user_id}').respond(201, json=self.user_json)
    ...
    async def test_delete_user(self):
        user_id = 1
        delete_user_mock = respx.delete(
            f'{BASE_URL}/api/v4/users/{user_id}').respond(204)
